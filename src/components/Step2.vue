...
export default {
  ...
  methods: {
    ...
    async handleDocSelect(item) {
      this.tableData.map(x => x.selected = false);
      item.selected = true;
      this.$setRouter(`?specId=${item.specId}&docId=${item.docId}`);
      this.resetSelections();
      const [error, result] = await api.getPersonnelDate(item);
      this.$processResponse(error, result);
      if (error) return;
      this.selectedItem.id = item.docId;
      this.selectedItem.name = `${item.family} ${item.name} ${item.ot}`;
        ...
        <table :class="$style.mainTable">
          ...
          <tbody>
            <tr
              ...
              @click="handleDocSelect(item)"
            >
              <td :class="$style.overflowWrap">
                <span
                  :class="$style.textOverflow"
                  v-text="`${item.family} ${item.name} ${item.ot}`"
                ></span>
              </td>
            </tr>
          </tbody>
